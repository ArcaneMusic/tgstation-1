...
/obj/machinery/newscaster
	...
	var/alert = FALSE
	/// The newcaster's index among all newscasters (GLOB.all_newscasters).
	var/unit_number = 0
...
/obj/machinery/newscaster/New()
	GLOB.all_newscasters += src
	unit_number = length(GLOB.all_newscasters)
...
/obj/machinery/newscaster/ui_act(action, list/params)
	...
	switch(action)
		...
		if("headlines")
			...
			screen = NEWSCASTER_HEADLINES
		if("channel")
			var/datum/feed_channel/feed_channel = locateUID(params["uid"])
			if(!istype(feed_channel))
				return
			if(screen == NEWSCASTER_CHANNEL && viewing_channel == feed_channel)
				return FALSE
			screen = NEWSCASTER_CHANNEL
			viewing_channel = feed_channel
		if("attach_photo")
		...
		if("censor_channel")
			if(is_security && !get_scanned_user(usr)["security"])
				...
				return
			var/datum/feed_channel/censored_feed_channel = locateUID(params["uid"])
			if(!istype(censored_feed_channel))
				return
			if(censored_feed_channel.admin_locked && !usr.can_admin_interact())
				set_temp("This channel has been locked by CentComm and thus cannot be (un)censored.", "danger")
				return
			censored_feed_channel.censored = !censored_feed_channel.censored
			...
			switch(id)
				if("create_channel", "manage_channel")
					...
					if(id == "manage_channel")
						var/datum/feed_channel/managed_feed_channel = locateUID(arguments["uid"])
						if(!istype(managed_feed_channel) || !managed_feed_channel.can_modify(usr, get_scanned_user(usr)["name"]))
							return
		...
		if(UI_MODAL_ANSWER)
			switch(id)
				if("create_channel", "manage_channel")
					...
					//
					var/datum/feed_channel/altered_feed_channel = null
					if(id == "create_channel") // Channel creation
						...
						if(GLOB.news_network.get_channel_by_author(author))
							...
							return
						altered_feed_channel = new
						GLOB.news_network.channels += altered_feed_channel
						SSblackbox.record_feedback("amount", "newscaster_channels", 1)
						// Redirect
						screen = NEWSCASTER_CHANNEL
						viewing_channel = altered_feed_channel
					else if (id == "manage_channel") // Channel management
						altered_feed_channel = locateUID(arguments["uid"])
						if(!altered_feed_channel || !altered_feed_channel.can_modify(usr, get_scanned_user(usr)["name"]))
							return
					// Add/update the information
					altered_feed_channel.channel_name = copytext(name, 1, CHANNEL_NAME_MAX_LENGTH)
					altered_feed_channel.description = copytext(description, 1, CHANNEL_DESC_MAX_LENGTH)
					altered_feed_channel.icon = usr.can_admin_interact() ? icon : "newspaper"
					altered_feed_channel.author = usr.can_admin_interact() ? author : scanned_user
					altered_feed_channel.is_public = public
					altered_feed_channel.admin_locked = usr.can_admin_interact() && admin_locked
					set_temp("Channel [altered_feed_channel.channel_name] created.", "good")
				if("create_story")
					var/author = trim(arguments["author"])
					...
					var/datum/feed_channel/parent_feed_channel = GLOB.news_network.get_channel_by_name(channel)
					if(!parent_feed_channel || !parent_feed_channel.can_publish(usr, user_name))
						return
					...
					// Register it
					parent_feed_channel.add_message(FM)
					SSblackbox.record_feedback("amount", "newscaster_stories", 1)
					var/announcement = parent_feed_channel.get_announce_text(title)
					// Announce it
					for(var/nc in GLOB.all_newscasters)
						var/obj/machinery/newscaster/NC = nc
						NC.alert_news(announcement)
					// Redirect and eject photo
					LAZYINITLIST(last_views[user_name])
					last_views[user_name][parent_feed_channel.UID()] = world.time
					screen = NEWSCASTER_CHANNEL
					viewing_channel = parent_feed_channel
					eject_photo(usr)
					set_temp("Story published to channel [parent_feed_channel.channel_name].", "good")
				if("wanted_notice")
					if(id == "wanted_notice" && !(is_security || usr.can_admin_interact()))
						return
					var/author = trim(arguments["author"])
...
/obj/machinery/newscaster/proc/print_newspaper_finish()
	...
	for(var/feed_channel in GLOB.news_network.channels)
		var/datum/feed_channel/recipient_channel = feed_channel
		NP.news_content += recipient_channel
